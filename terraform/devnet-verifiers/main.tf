terraform {
  backend "s3" {
    ...
    key    = "services/axelarscan/devnet-verifiers/terraform.tfstate"
    region = "us-east-2"
  }
  required_providers {
    aws = {
...
data "aws_api_gateway_domain_name" "devnet_verifiers" {
  domain_name = "devnet-verifiers.api.axelarscan.io"
}
...
resource "aws_apigatewayv2_stage" "devnet_verifiers" {
  api_id      = aws_apigatewayv2_api.api.id
  auto_deploy = true
  name        = var.environment
}

resource "aws_apigatewayv2_api_mapping" "devnet-verifiers" {
  api_id          = aws_apigatewayv2_api.api.id
  domain_name     = data.aws_api_gateway_domain_name.devnet_verifiers.id
  stage           = aws_apigatewayv2_stage.devnet_verifiers.id
