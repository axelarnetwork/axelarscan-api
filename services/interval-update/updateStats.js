        ...
        new Promise(async resolve => {
          switch (d) {
            ...
            case 'transfersTopUsers':
              resolve(
                await METHODS.transfersTopUsers({ size: 100, forceCache: true })
              );
              break;
            case 'transfersTopUsersByVolume':
              resolve(
                await METHODS.transfersTopUsers({
                  orderBy: 'volume',
                  size: 100,
                  forceCache: true,
                })
            ...
            case 'GMPStatsAVGTimes':
              resolve(
                await METHODS.GMPStatsAVGTimes({
                  fromTime: moment()
                    .subtract(1, 'months')
                    .startOf('day')
                    .unix(),
            ...
            case 'GMPTopUsers':
              resolve(
                await METHODS.GMPTopUsers({ size: 100, forceCache: true })
              );
              break;
            case 'GMPTopITSUsers':
              resolve(
                await METHODS.GMPTopUsers({
                  assetType: 'its',
                  size: 100,
                  forceCache: true,
                })
            ...
            case 'GMPTopITSAssets':
              resolve(
                await METHODS.GMPTopITSAssets({ size: 100, forceCache: true })
              );
              break;
            case 'GMPTopITSAssetsByVolume':
              resolve(
                await METHODS.GMPTopITSAssets({
                  orderBy: 'volume',
                  size: 100,
                  forceCache: true,
                })
