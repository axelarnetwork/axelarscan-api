...
const getTokenConfig = async (symbol, assetsData, noRequest = false) => {
  ...
  if (!tokenData) {
    const customTokenInContracts = _.head(
      toArray(
        toArray(custom_contracts).map(d =>
          toArray(d.assets).find(a => a.symbol === symbol && a.coingecko_id)
        )
      )
    );
  ...
  // custom token of tvl
  if (!tokenData) {
    const customToken = toArray(custom_tokens).find(
      d => d.symbol === symbol && d.coingecko_id
    );
  ...
  if (!tokenData && !noRequest) {
    tokenData =
      (await getAssetData(symbol, assetsData)) ||
      (await getITSAssetData(symbol, assetsData));
  }
...
module.exports = async ({
  ...
  symbols = _.uniq(
    toArray(_.concat(symbols, symbol)).map(s =>
      s.startsWith('burned-') ? s.replace('burned-', '') : s
    )
  );

  // valid when some symbols are in config
  const isSymbolsValid =
    toArray(
      await Promise.all(
        symbols.map(
          s =>
  ...
  if (tokensData.findIndex(d => d.coingecko_id) > -1) {
    ...
    if (tokensData.findIndex(d => !isNumber(d.price)) > -1) {
      ...
      if (cacheId) {
                  ...
                  must: [
                    {
                      range: {
                        updated_at: {
                          gte: moment().subtract(300, 'seconds').valueOf(),
                        },
                      },
                    },
                  ],
        ...
        if (
          toArray(data).length >=
          ids.length - (ids.length > 50 && !forceCache ? 3 : 0)
        ) {
          response = Object.fromEntries(
            data.flatMap(d => Object.entries(d.data))
          );
        }
      }
      ...
      if (!response) {
        ...
        if (response && !response.error) {
          ...
          if (cacheId) {
            response = await readCache(
              cacheId,
              4 * 3600,
              TOKEN_PRICE_COLLECTION
            );
          } else {
            const data = await readMultipleCache(
              ids,
              4 * 3600,
              TOKEN_PRICE_COLLECTION
            );
