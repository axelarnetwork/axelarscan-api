...
const { alert_asset_escrow_value_threshold, alert_asset_value_threshold } = {
  ...getTVLConfig(),
};
...
module.exports = async () => {
          ...
          must: [
            ...
            { exists: { field: 'data.tvl' } },
            {
              range: {
                updated_at: {
                  gt: moment()
                    .subtract(MAX_INTERVAL_UPDATE, 'seconds')
                    .valueOf(),
                },
              },
            },
  ...
  data = _.orderBy(
    toArray(data)
      .flatMap(d => toArray(d.data))
      .map(d => ({
        ...d,
        value_diff: toNumber(
          (d.total - (d.total_on_contracts + d.total_on_tokens)) *
            (d.percent_diff_supply / 100) *
            d.price
        ),
        tvl: Object.fromEntries(
          Object.entries(d.tvl).map(([k, v]) => [
            k,
            {
              ...v,
              is_abnormal_supply:
                !getChainData(k)?.no_alert_tvl && v.is_abnormal_supply,
              value_diff: toNumber(
                (v.escrow_balance || v.supply) *
                  (v.percent_diff_supply / 100) *
                  d.price
              ),
            },
          ])
  ...
  const toAlertData = data.filter(
    d =>
      (d.is_abnormal_supply && d.value_diff > alert_asset_value_threshold) || // value of supply diff > threshold
      Object.entries(d.tvl).find(
        ([k, v]) =>
          v.is_abnormal_supply &&
          v.value_diff > alert_asset_escrow_value_threshold
      ) // value of escrow balance diff > threshold
  );
  ...
  if (toAlertData.length > 0) {
      ...
      toAlertData.map(
        d =>
          new Promise(async resolve => {
            const { native_chain, symbol } = {
              ...(await getAssetData(d.asset, assetsData)),
            };

            // urls to initial check
            const appURLs = [
              `${getAppURL()}/tvl`,
              `${getAppURL()}/transfers/search?asset=${d.asset}&fromTime=${moment().subtract(24, 'hours').unix()}&toTime=${moment().unix()}&sortBy=value`,
            ];
                    ...
                    links: _.uniq(
                      toArray(
                        _.concat(
                          ...
                          toArray(d.tvl[native_chain]).flatMap(v =>
                            _.concat(
                              v.url,
                              v.escrow_addresses_urls,
                              v.supply_urls
                            )
                          ),
                          appURLs
    ...
    native_on_evm_total_status =
      details.findIndex(
        d => d.native_on === 'evm' && isNumber(d.percent_diff_supply)
      ) > -1
        ? 'alert'
        : 'ok';
    native_on_evm_escrow_status =
      details.findIndex(
        d =>
          d.native_on === 'evm' &&
          toArray(d.chains).findIndex(c => isNumber(c.percent_diff_supply)) > -1
      ) > -1
        ? 'alert'
        : 'ok';
    native_on_cosmos_evm_escrow_status =
      details.findIndex(
        d => d.native_on === 'cosmos' && isNumber(d.percent_diff_supply)
      ) > -1
        ? 'alert'
        : 'ok';
    ...
    // details
    const evmDetails =
      [native_on_evm_total_status, native_on_evm_escrow_status].findIndex(
        s => s !== 'ok'
      ) > -1
    ...
    summary = toArray(_.concat(evmDetails, cosmosDetails))
      .map(d => d.symbol)
      .join(', ');
