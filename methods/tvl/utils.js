...
const normalizeParams = params => {
  if (params) {
    params.assets = _.uniq(
      _.concat(toArray(params.assets), toArray(params.asset))
    );
    params.chains = _.uniq(
      _.concat(toArray(params.chains), toArray(params.chain))
    );
...
const generateDenomForAsset = d =>
  `${d.decimals === 6 ? 'u' : ''}${toCase(d.symbol, 'lower')}${d.decimals === 18 ? '-wei' : ''}`;
...
const isAssetsEquals = (a, b, isNativeToken = false) => {
  ...
  return (
    equalsIgnoreCase(a.symbol, b.symbol) ||
    (isNativeToken &&
      (equalsIgnoreCase(a.symbol, `W${b.symbol}`) ||
        equalsIgnoreCase(b.symbol, `W${a.symbol}`)))
  );
};
...
const getCustomContractsData = (chainData, assetData) => {
  ...
  return toArray(custom_contracts)
    .filter(c => c.address) // has contract address
    .filter(c => !chainData || equalsIgnoreCase(c.chain, chainData.id)) // filter by chain
    .filter(
      c =>
        !assetData ||
        toArray(c.assets).findIndex(
          a => a.address && isAssetsEquals(a, assetData)
        ) > -1
    ) // filter by contain asset
    .map(c => ({
      ...c,
      assets: toArray(c.assets).filter(
        a => a.address && (!assetData || isAssetsEquals(a, assetData))
      ), // filter by asset
    }))
    .filter(c => toArray(c.assets).length > 0); // filter has assets
};

const getCustomContractsAssetsData = (
  chainData,
  assetData,
  gatewayAssetsData
) => {
  const customAssetsFromContracts = getCustomContractsData(
    chainData,
    assetData
  ).flatMap(c =>
    toArray(c.assets)
      .filter(
        a => toArray(gatewayAssetsData).findIndex(d => isAssetsEquals(d, a)) < 0
  ...
  return Object.values(
    _.groupBy(_.uniqBy(customAssetsFromContracts, 'key'), 'symbol')
  ).map(assetsData => {
    ...
    .filter(
      a =>
        ...
        isAssetsEquals(
          ...
          assetData,
          chainData
            ? a.addresses[chainData.id].address === ZeroAddress
            : undefined
        )
    ); // filter by asset
};
...
const getCustomTokensAssetsData = (chainData, assetData, gatewayAssetsData) => {
  const customTokens = getCustomTokensData(chainData, assetData).filter(
    a => gatewayAssetsData.findIndex(d => isAssetsEquals(d, a)) < 0
  ); // filter not gateway asset
...
const getTVLAssets = async params => {
  ...
  // gateway
  const gatewayAssetsData = (await getAssets()).map(d => ({
    ...d,
    type: 'gateway',
  }));

  // custom contracts
  const customContractsAssetsData = getCustomContractsAssetsData(
    undefined,
    undefined,
    gatewayAssetsData
  );
  ...
  // custom tokens
  const customTokensAssetsData = getCustomTokensAssetsData(
    undefined,
    undefined,
    gatewayAssetsData
  );
  ...
  const itsAssetsData = customAssetsOnly
    ...
    : (await getITSAssets()).map(d => {
        ...
        d.addresses = Object.fromEntries(
          Object.entries({ ...d.chains }).map(([k, v]) => [
            k,
            {
              symbol: v.symbol,
              token_manager_address: v.tokenManager,
              token_manager_type: v.tokenManagerType,
              [getChainType(k) === 'cosmos' ? 'ibc_denom' : 'address']:
                v.tokenAddress,
            },
          ])
        );
  ...
  const assetsData = _.concat(
    ...
    Object.values(
      _.groupBy(
        _.concat(customContractsAssetsData, customTokensAssetsData),
        'id'
      )
    ).map(assetsData => ({
      ..._.head(assetsData),
          ...
          d =>
            new Promise(async resolve => {
              ...
              for (const asset of assets) {
                if (
                  (await getAssetData(asset, d)) ||
                  (await getITSAssetData(asset, d))
                ) {
                  includes = true;
                  break;
                }
...
const getTVLChains = (params, types) => {
  ...
  const chainsData = getChains(types).filter(
    d => !d.no_tvl && (d.chain_type === 'cosmos' || d.gateway?.address)
  );
...
const isNativeTokenAddress = address =>
  [ZeroAddress, '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE'].includes(address);
