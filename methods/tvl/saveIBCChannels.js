...
module.exports = async () => {
  ...
  while (nextKey) {
    ...
    const { channels, pagination } = {
      ...(await request(getLCDInstance(), {
        path: '/ibc/core/channel/v1/channels',
        params: isString(nextKey) ? { 'pagination.key': nextKey } : undefined,
      })),
    };

    // merge channels and uniq by channel_id
    allChannels = _.uniqBy(
      toArray(_.concat(allChannels, channels)),
      'channel_id'
    );
  ...
  // query all channels
  const { data } = {
    ...(await read(IBC_CHANNEL_COLLECTION, { match_all: {} }, { size: 1000 })),
  };
      ...
      allChannels.map(
        channel =>
          new Promise(async resolve => {
            const { channel_id, port_id, version, counterparty, updated_at } = {
              ...channel,
            };
            ...
            if (
              ...
              timeDiff(updated_at * 1000, 'minutes') > 240
            ) {
              // get channel state
              const response = await request(getLCDInstance(), {
              ...
              if (chain_id) {
                ...
                escrow_address =
                  getBech32Address(`${version}\x00${port_id}/${channel_id}`) ||
                  escrow_address;
                ...
                if (counterparty && prefix_address) {
                  ...
                  counterparty.escrow_address = getBech32Address(
                ...
                await write(
                  ...
                  channel_id,
                  {
                    ...
                    counterparty,
                    escrow_address,
                    latest_height: isNumber(
                      client_state?.latest_height?.revision_height
                    )
                      ? toNumber(client_state.latest_height.revision_height)
                      : undefined,
