...
module.exports = async params => {
  ...
  const isAllChains =
    chainsData.length >=
    getTVLChains(undefined, ['evm', 'cosmos', 'amplifier']).length;
  const isAllCosmosChains =
    cosmosChainsData.length >= getTVLChains(undefined, ['cosmos']).length;
  ...
  if (!forceCache ? assetsData.length > 1 && isAllChains : isIntervalUpdate) {
    ...
    if (assetsData.length > 200) {
              ...
              must: [
                {
                  range: {
                    updated_at: {
                      gte: moment().subtract(CACHE_AGE, 'seconds').valueOf(),
                    },
                  },
                },
              ],
      ...
      data = await readMultipleCache(
        assetsData.map(d => d.id),
        CACHE_AGE,
        TOKEN_TVL_COLLECTION
      );
    }
  ...
  for (const assetData of assetsData) {
    ...
    const isCanonicalITS =
      type === 'its' &&
      Object.values({ ...addresses }).findIndex(d =>
        d.token_manager_type?.startsWith('lockUnlock')
      ) > -1;

    let tvlsByChain = Object.fromEntries(
      (
        await Promise.all(
          chainsData.map(
            d =>
              new Promise(async resolve => {
                ...
                const { url, address_path, contract_path, asset_path } = {
                  ...explorer,
                };
                    ...
                    try {
                      ...
                      if (address) {
                        ...
                        const isLockUnlock =
                          type === 'its' &&
                          token_manager_address &&
                          token_manager_type?.startsWith('lockUnlock');
                        // get balance of this asset on token manager
                        const tokenManagerBalance = isLockUnlock
                          ? toNumber(
                              await getBalance(
                                chain,
                                token_manager_address,
                        ...
                        const supply =
                          (!isNative || type === 'its') && !is_custom
                            ? isLockUnlock
                              ? tokenManagerBalance
                              : toNumber(
                                  await getTokenSupply(chain, contractData)
                                )
                            : 0;

                        // custom contracts with their asset balance
                        const customContractsBalances = await Promise.all(
                          getCustomContractsData(d, assetData).map(
                            c =>
                              new Promise(async resolve =>
                                resolve({
                                  address: c.address,
                        ...
                        const customTokensSupply = await Promise.all(
                          getCustomTokensData(d, assetData).map(
                            a =>
                              new Promise(async resolve => {
                        ...
                        tvlData = {
                          ...
                          supply,
                          total:
                            isNativeOnCosmos || isNativeOnAmplifier
                              ? 0
                              : gatewayBalance + supply,
                          // custom contracts
                          custom_contracts_balance: customContractsBalances,
                          total_balance_on_custom_contracts: _.sumBy(
                            customContractsBalances,
                            'balance'
                          ),
                          // custom tokens
                          custom_tokens_supply: customTokensSupply,
                          total_supply_of_custom_tokens: _.sumBy(
                            customTokensSupply,
                            'supply'
                          ),
                          // link
                  ...
                  case 'cosmos':
                    try {
                      ...
                      if (ibc_denom) {
                        ...
                        if (
                          chain !== axelar.id &&
                          toArray(prefix_chain_ids).length > 0
                        ) {
                          let wasRun;
                          ...
                          while (!wasRun) {
                            ...
                            const { channelId } = {
                              ...axelarS3ChainsConfig?.chains?.[chain]?.config
                                ?.ibc?.fromAxelar,
                            };
                            ...
                            if (
                              ...
                              !data.find(
                                d =>
                                  timeDiff(d.updated_at * 1000) >
                                  IBC_CHANNELS_CACHE_AGE
                              )
                            ) {
                              // filter ibc channels by channelId
                              ibcChannels = _.orderBy(
                                data.filter(
                                  d =>
                                    (!channelId && !isSecretChain(chain)) ||
                              ...
                              escrowAddresses = toArray(
                                ibcChannels.map(d => d.escrow_address)
                              );
                              sourceEscrowAddresses = toArray(
                                ibcChannels.map(
                                  d => d.counterparty?.escrow_address
                                )
                              );
                        ...
                        const escrowBalance = isSecretChain(chain)
                          ? undefined
                          : _.sum(
                              await Promise.all(
                                toArray(escrowAddresses).map(
                                  a =>
                                    new Promise(async resolve =>
                                      resolve(
                                        toNumber(
                        ...
                        const sourceEscrowBalance = _.sum(
                          await Promise.all(
                            toArray(sourceEscrowAddresses).map(
                              a =>
                                new Promise(async resolve =>
                                  resolve(
                                    toNumber(
                        ...
                        // flags
                        const isNativeOnCosmos =
                          isNative && chain !== axelar.id;
                        const isNotNativeOnAxelar =
                          !isNative && chain === axelar.id;

                        let supply = isNative
                          ? chain !== axelar.id
                            ? sourceEscrowBalance
                            : 0 // use source escrow balance when native on cosmos chain
                          : toArray(escrowAddresses).length > 0
                            ? await getIBCSupply(chain, contractData)
                            : 0; // get ibc supply on this chain
                        ...
                        let total = isNotNativeOnAxelar
                          ...
                          : isNativeOnCosmos
                            ? await getIBCSupply(axelar.id, {
                                ...contractData,
                                ibc_denom: denom,
                              }) // when native on cosmos, get ibc supply on axelar
                            : isSecretSnipChain(chain)
                              ? escrowBalance
                              : 0; // use escrow balance when secret-snip
                        ...
                        const totalSupply = isNativeOnCosmos
                          ? toNumber(
                              await getIBCSupply(axelar.id, contractData)
                            )
                          : 0;

                        const percentDiffSupply = isNativeOnCosmos
                          ? // compare total supply on axelar with source escrow balance
                            totalSupply > 0 && sourceEscrowBalance > 0
                            ? (Math.abs(sourceEscrowBalance - totalSupply) *
                                100) /
                              sourceEscrowBalance
                            : null
                          : // compare supply with escrow balance
                            supply > 0 && escrowBalance > 0
                            ? (Math.abs(escrowBalance - supply) * 100) /
                              escrowBalance
                            : null;
                        ...
                        tvlData = {
                          ...
                          url:
                            isNativeOnCosmos &&
                            toArray(sourceEscrowAddresses).length > 0
                              ...
                              : ibc_denom.includes('/') &&
                                ...
                                : toArray(escrowAddresses).length > 0
                                  ? `${axelar.explorer?.url}${axelar.explorer?.address_path?.replace('{address}', isSecretSnipChain(chain) ? _.head(escrowAddresses) : _.last(escrowAddresses))}`
                                  : null,
                          escrow_addresses_urls: isNativeOnCosmos
                            ? _.reverse(
                                _.cloneDeep(toArray(sourceEscrowAddresses))
                              ).flatMap(a => [
                            ...
                            : _.reverse(
                                _.cloneDeep(toArray(escrowAddresses))
                              ).flatMap(a =>
                                toArray([
                                  `${axelar.explorer?.url}${axelar.explorer?.address_path?.replace('{address}', a)}`,
                                  denom &&
                                    `${axelarLCD}/cosmos/bank/v1beta1/balances/${a}/by_denom?denom=${encodeURIComponent(denom)}`,
                          ...
                          supply_urls:
                            ...
                            toArray(escrowAddresses).length > 0
                              ? [
                                  `${LCD}/cosmos/bank/v1beta1/supply/${encodeURIComponent(ibc_denom)}`,
                                  `${LCD}/cosmos/bank/v1beta1/supply`,
                                ]
                              : [],
                          // status
                          success:
                            isNumber(isNotNativeOnAxelar ? total : supply) ||
                            d.unstable,
                        };
                      }
                    } catch (error) {}
                    ...
                    try {
                      ...
                      if (address) {
                        ...
                        const gatewayBalance =
                          type === 'gateway'
                            ? toNumber(
                                await getRPCs(chain)?.getBalance(
                                  gateway.address,
                                  contractData
                                )
                              )
                        ...
                        const supply =
                          (!isNative || type === 'its') && !is_custom
                            ? isLockUnlock
                              ? tokenManagerBalance
                              : toNumber(
                                  await getRPCs(chain)?.getTokenSupply(
                                    contractData
                                  )
                                )
                            : 0;
                        ...
                        tvlData = {
                          ...
                          supply,
                          total:
                            isNativeOnEVM || isNativeOnCosmos
                              ? 0
                              : gatewayBalance + supply,
                          // link
    ...
    if (type === 'gateway' && !isNativeOnAxelar) {
      tvlsByChain = Object.fromEntries(
        Object.entries(tvlsByChain).map(([k, v]) => {
          if (k === axelar.id) {
            if (!v.total) {
              v.supply = 0;
            } else {
              ...
              v.supply =
                ...
                _.sum(
                  Object.entries(tvlsByChain)
                    ...
                    .map(([k, v]) =>
                      toNumber(isSecretSnipChain(k) ? v.total : v.supply)
                    )
                );
            }
          }
    ...
    let totalOnEVM = !(type === 'gateway' || isCanonicalITS)
      ? 0
      : _.sum(
          Object.entries(tvlsByChain)
            .filter(
    ...
    const totalOnCosmos = !(type === 'gateway' || isCanonicalITS)
      ? 0
      : _.sum(
          Object.entries(tvlsByChain)
            .filter(
              ([k, v]) => getChainType(k) === 'cosmos' && k !== native_chain
            )
            .map(([k, v]) =>
              toNumber(
                isAllCosmosChains
                  ? isNativeOnCosmos || isSecretSnipChain(k)
                    ? v.supply
                    : v.total
                  : v.escrow_balance
              )
    ...
    let totalOnAmplifier = !(type === 'gateway' || isCanonicalITS)
      ? 0
      : _.sum(
          Object.entries(tvlsByChain)
            .filter(
    ...
    const totalOnContracts = !(type === 'gateway')
      ? 0
      : _.sum(
          Object.entries(tvlsByChain)
    ...
    const totalOnTokens = !(type === 'gateway')
      ? 0
      : _.sum(
          Object.entries(tvlsByChain)
            .filter(([k, v]) => getChainType(k) === 'evm')
    ...
    const total =
      totalOnContracts +
      totalOnTokens +
      (isNativeOnCosmos || isNativeOnAxelar
        ? totalOnEVM + totalOnCosmos + totalOnAmplifier
        : type === 'its'
          ? isCanonicalITS
            ? _.sum(
                Object.values(tvlsByChain).map(v =>
                  toNumber(v.token_manager_balance)
                )
              ) // lockUnlock token manager balances
            : toNumber(await getTokenCirculatingSupply(coingecko_id)) // circulating supply from coingecko
          : _.sum(
              Object.values(tvlsByChain).map(v =>
                toNumber(isNativeOnEVM ? v.gateway_balance : v.total)
              )
            )); // gateway balances for native on evm, otherwise total
    ...
    const evmEscrowAddress = isNativeOnCosmos
      ? getBech32Address(
          isNativeOnAxelar
            ? asset
            : `ibc/${toHash(`transfer/${_.last(tvlsByChain[native_chain]?.ibc_channels)?.channel_id}/${asset}`)}`,
          axelar.prefix_address,
          32
        )
      : undefined;

    // get axelar asset balance of evm escrow address
    const evmEscrowBalance = evmEscrowAddress
      ? toNumber(
          await getCosmosBalance(
            axelar.id,
            evmEscrowAddress,
            getContractData(assetData, axelar)
          )
        )
      : 0;

    // debug urls of evm escrow address
    const evmEscrowURLs = !evmEscrowAddress
      ? undefined
      : [
    ...
    const percentDiffSupply = evmEscrowAddress
      ? // compare evm escrow balance with total on evm
        evmEscrowBalance > 0 && totalOnEVM > 0
        ? (Math.abs(evmEscrowBalance - totalOnEVM) * 100) / evmEscrowBalance
        : null
      : // compare total with total on evm + total on cosmos
        total > 0 && totalOnEVM + totalOnCosmos + totalOnAmplifier > 0
        ? (Math.abs(
            total -
              (totalOnContracts + totalOnTokens) -
              (totalOnEVM + totalOnCosmos + totalOnAmplifier)
          ) *
            100) /
          (total - (totalOnContracts + totalOnTokens))
        : null;

    // price
    const pricesData = await getTokensPrice({
      symbol: is_custom && symbol ? symbol : asset,
    });
