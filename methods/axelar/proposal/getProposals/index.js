...
module.exports = async () => {
  ...
  while (nextKey) {
    // get all proposals
    const { proposals, pagination } = { ...await request(getLCDInstance(), { path: '/cosmos/gov/v1beta1/proposals', params: { 'pagination.key': isString(nextKey) ? nextKey : undefined } }) };

    data = _.orderBy(_.uniqBy(
      _.concat(data, await Promise.all(toArray(proposals).map(d => new Promise(async resolve => {
        // normalize
        ...
        d.content = {
          ...d.content,
          plan: d.content?.plan && { ...d.content.plan, height: toNumber(d.content.plan.height) },
        };
