...
module.exports = async params => {
  ...
  while (nextKey) {
    ...
    const { unbonding_responses, pagination } = { ...await request(getLCDInstance(height), { path: `/cosmos/staking/v1beta1/delegators/${address}/unbonding_delegations`, params: { 'pagination.key': isString(nextKey) ? nextKey : undefined } }) };

    data = _.orderBy(
      _.concat(data, toArray(unbonding_responses).flatMap(d => {
        const { entries } = { ...d };
