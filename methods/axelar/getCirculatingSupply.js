...
const calculateVesting = (
  ...
  const { total_unlock_percent, vesting_period, vesting_start, vesting_until } =
    { ...config };
  const { decimals } = { ...assetData };

  const total = parseFloat(
    toFixed((max_supply * total_unlock_percent) / 100, decimals)
  );
  const vestingStart = moment(vesting_start, 'YYYY-MM-DD')
    .startOf('day')
    .valueOf();
  const vestingUntil = moment(vesting_until, 'YYYY-MM-DD')
    .startOf('day')
    .valueOf();
        ...
        : timestamp <= vestingStart
          ...
          : // total * (vested time / total vesting time)
            parseFloat(
              toFixed(
                (total *
                  timeDiff(vestingStart, `${vesting_period}s`, timestamp)) /
                  timeDiff(vestingStart, `${vesting_period}s`, vestingUntil),
                decimals
              )
            ),
    config,
...
module.exports = async params => {
  ...
  switch (symbol) {
    ...
    case 'AMPLIFIER':
      ...
      const inflationRewards =
        totalSupply > max_supply
          ? parseFloat(toFixed(totalSupply - max_supply, decimals))
          : 0;
      const initialUnlocked = parseFloat(
        toFixed((max_supply * initial_unlocked_percent) / 100, decimals)
      );

      // getBlockTimestamp when specific height
      const timestamp = height
        ? await getBlockTimestamp(height)
        : moment().valueOf();

      // vesting
      const communitySale = calculateVesting(
        community_sale,
        assetData,
        timestamp
      );
      const communityPrograms = calculateVesting(
        community_programs,
        assetData,
        timestamp
      );
      const companyOperations = calculateVesting(
        company_operations,
        assetData,
        timestamp
      );
      ...
      circulatingSupply =
        inflationRewards +
        initialUnlocked +
        communitySale?.unlocked +
        communityPrograms?.unlocked +
        companyOperations?.unlocked +
        _backers?.unlocked +
        _team?.unlocked;
    ...
    default:
      if (denom) {
        ...
        const { total, total_on_evm, total_on_cosmos } = {
        ...
        circulatingSupply = isAxelarAsset
