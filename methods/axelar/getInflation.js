...
module.exports = async params => {
  ...
  const chainsData = getChains('evm').filter(
    d => !d.no_inflation && d.gateway?.address
  );

  let { uptimeRate, heartbeatRate, numEVMChains, unsubmittedVoteRates } = {
    ...params,
  };
  ...
  unsubmittedVoteRates =
    unsubmittedVoteRates || Object.fromEntries(chainsData.map(d => [d.id, 0]));
  ...
  return {
    ...
    inflation: parseFloat(
      toFixed(
        uptimeRate * tendermintInflationRate +
          heartbeatRate *
            keyMgmtRelativeInflationRate *
            tendermintInflationRate +
          externalChainVotingInflationRate *
            _.sum(Object.values(unsubmittedVoteRates).map(v => 1 - v)),
        6
      )
    ),
  };
