...
module.exports = async params => {
  ...
  const assetData = {
    ...((await getAssetData(symbol)) || (await getITSAssetData(symbol))),
  };
  const { denom, name } = { ...assetData };

  const isAXL = ['uaxl', 'uamplifier'].includes(denom);

  const [price, supplyData, totalSupply, totalBurned, inflation] =
    await Promise.all(
      [
        'price',
        'circulatingSupply',
          ...
          new Promise(async resolve => {
            switch (k) {
              case 'price':
                ...
                const { price } = {
                  ...(pricesData[symbol] ||
                    Object.values(pricesData).find(d => d.denom === symbol)),
                };
                resolve(price);
                break;
              ...
              case 'totalSupply':
                resolve(
                  isAXL
                    ? await getTotalSupply({ asset: denom, height, assetData })
                    : null
                );
                break;
              case 'totalBurned':
                resolve(
                  isAXL
                    ? await getTotalBurned({ height, assetsData: assetData })
                    : null
                );
                break;
              case 'inflation':
                resolve(
                  isAXL ? (await getInflation({ height }))?.inflation : null
                );
                break;
              default:
                resolve();
    ...
    case 'upbit':
      ...
      return ['KRW', 'USD', 'IDR', 'SGD', 'THB'].map(currencyCode => {
        ...
        const priceConverted =
          price *
          (exchangeRates?.[currency] && currency !== CURRENCY
            ? exchangeRates[currency].value / exchangeRates[CURRENCY].value
            : 1);
